...
const Onboarding = ({ onComplete }) => {
      ...
      case 1:
        return (
          <motion.div key="step1" variants={variants} initial="enter" animate="center" exit="exit" style={{ textAlign: 'center', width: '100%', maxWidth: '400px' }}>
            ...
            <h2 style={{ fontSize: '24px', fontWeight: 900, marginBottom: '10px', textTransform: 'uppercase' }}>WASAFI ANTHEM</h2>
            <p style={{ color: 'var(--text-gray)', marginBottom: '30px' }}>Which Wasafi hit defines your vibe?</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              {songs.map((song) => (
                ...
                >
                  <div style={{ fontSize: '14px', marginBottom: '2px' }}>{song.title}</div>
                  <div style={{ fontSize: '10px', color: favoriteSong === song.title ? 'var(--primary-red)' : 'var(--text-gray)' }}>{song.artist}</div>
                </button>
              ))}
            </div>
          </motion.div>
      ...
      case 2:
        return (
          <motion.div key="step2" variants={variants} initial="enter" animate="center" exit="exit" style={{ textAlign: 'center', width: '100%', maxWidth: '400px' }}>
            ...
            <h2 style={{ fontSize: '24px', fontWeight: 900, marginBottom: '10px', textTransform: 'uppercase' }}>DAILY FIX</h2>
            <p style={{ color: 'var(--text-gray)', marginBottom: '30px' }}>Where do you get your daily Wasafi fix?</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              {shows.map((show) => (
                <button
                  key={show}
      ...
      case 3:
        return (
          <motion.div key="step3" variants={variants} initial="enter" animate="center" exit="exit" style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: '24px', fontWeight: 900, marginBottom: '10px' }}>WCB LIONS FAMILY</h2>
            <p style={{ color: 'var(--text-gray)', marginBottom: '20px', maxWidth: '300px' }}>
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backgroundColor: 'var(--bg-black)', padding: '20px' }}>
